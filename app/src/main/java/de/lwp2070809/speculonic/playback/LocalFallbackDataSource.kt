    ...
    override fun open(dataSpec: DataSpec): Long {
        ...
        if (songId != null) {
            ...
            if (!localUri.isNullOrBlank()) {
                try {
                    val safUri = localUri.toUri()
                    
                    val safDataSpec = dataSpec.withUri(safUri)
                    val bytesRead = contentDataSource.open(safDataSpec)
                    activeDataSource = contentDataSource
                    LogManager.d("LocalFallbackDataSource: Successfully opened SAF for $songId: $localUri")
                    return bytesRead
                } catch (e: Exception) {
                    LogManager.w("LocalFallbackDataSource: Failed to open SAF for $songId, falling back to network. Error: ${e.message}")
